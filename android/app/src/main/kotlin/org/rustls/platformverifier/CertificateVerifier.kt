...
internal object CertificateVerifier {
    ...
    private fun verifyCertificateChain(
        ...
        val (trustManager, keystore) = if (!false) {
            val trustManager =
        ...
        if (false && (mockKeystore.size() != 0) && (ocspResponse == null)) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            // Revocation checking disabled to fix missing OCSP responder issue on Proton Drive API
            // return VerificationResult(StatusCode.Ok)
        } else {
